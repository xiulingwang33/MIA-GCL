...
def train_mia(encoder_model, contrast_model, data,edges_train_index, optimizer,device,sigma):
    encoder_model.train()
    optimizer.zero_grad()
    z, z1, z2,edge_index1,edge_index2 = encoder_model(data.x,data.edge_index,edges_train_index, data.edge_attr)
    h1, h2 = [encoder_model.project(x) for x in [z1, z2]]
    ...
    grads = [torch.zeros(p.shape).to(device) for p in encoder_model.parameters()]
    # print((grads))
    igrad = torch.autograd.grad(loss, encoder_model.parameters(), retain_graph=True)
    # print(igrad)
    # for i in igrad:
    #     print(i.size())
    ...
    for i in range(len(grads)):
        # print(grads[i])
        grads[i] += sigma * (torch.randn_like(grads[i]).to(device))
        # print(grads[i])
        grads[i].detach_()
        # exit()

    p_list = [p for p in encoder_model.parameters()]
